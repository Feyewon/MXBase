...
#import "DCMediaPhotoLibraryManager.h"
#import <AssetsLibrary/AssetsLibrary.h>
#import "UIImage+Addition.h"
#import "MXMacro.h"

#define kQUScreenScale [UIScreen mainScreen].scale

@interface DCMediaPhotoLibraryManager ()
@property (nonatomic, strong) ALAssetsLibrary *assetLibrary;
@end

@implementation DCMediaPhotoLibraryManager

+ (instancetype)sharedManager {
    static DCMediaPhotoLibraryManager *mg = nil;
        ...
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            ...
            dispatch_sync(dispatch_get_main_queue(), ^{
                if (success && completion) {
                    RUN(completion, nil, nil);
                } else if (error) {
                    RUN(completion, nil, error);
                }
            });
        }];
    } else {
...
- (void)getPhotoWithAsset:(PHAsset *)asset thumbnailWidth:(CGFloat)width
    ...
    PHImageRequestID imageRequestID = [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:imageSize contentMode:PHImageContentModeAspectFill options:option resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
        ...
        if ([info objectForKey:PHImageResultIsInCloudKey] && !result && networkAccessAllowed) {
            ...
            [[PHImageManager defaultManager] requestImageDataForAsset:asset options:options resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                ...
                resultImage = [resultImage imageByResizeToSize:imageSize contentMode:UIViewContentModeScaleAspectFill];
                if (resultImage) {
...
- (PHImageRequestID)getPhotoDataWithAsset:(id)asset
    ...
    return [[PHImageManager defaultManager] requestImageDataForAsset:asset options:options resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
        if ([dataUTI isEqualToString:@"public.heic"]) { //live photo 资源需要转成JPG
...
#pragma mark --- 获取相册资源
// 相册封面
- (void)getPostImageWithAlbumModel:(DCMediaAlbumModel *)model completion:(void (^)(UIImage *))completion {
...
- (void)getCameraRollAssetWithallowPickingVideo:(BOOL)allowPickingVideo
    ...
    [self getAssetsFromFetchResult:fetchResult allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage completion:^(NSArray<DCMediaAssetModel *> *models) {
        photoArray = (NSArray *)models;
    }];
    RUN(completion, photoArray, videoCount);
}
    ...
    for (PHFetchResult *fetchResult in allAlbums) {
        for (PHAssetCollection *collection in fetchResult) {
            ...
            if ([self isCameraRollAlbum:collection.localizedTitle]) {
                DCMediaAlbumModel *model = [[DCMediaAlbumModel alloc] initWithFetchResult:fetchResult albumName:collection.localizedTitle];
...
- (void)getAssetsFromFetchResult:(PHFetchResult *)fetchResult
    ...
    [fetchResult enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj != nil) {
            DCMediaAssetModel *model = [self assetModelWithAsset:obj allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
            if (model) {
                [photoArr addObject:model];
            }
        }
    }];
    RUN(completion,photoArr);
}
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result
    ...
    DCMediaAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
...
- (PHImageRequestID)getVideoWithAsset:(PHAsset *)asset completion:(void (^)(AVAsset * avAsset, NSDictionary * info))completion {
    return [self getVideoWithAsset:asset progressHandler:nil completion:completion];
}
...
- (PHImageRequestID)getVideoWithAsset:(PHAsset *)asset
    ...
    options.version = PHVideoRequestOptionsVersionCurrent;
    options.progressHandler = ^(double progress, NSError *error, BOOL *stop, NSDictionary *info) {
        RUN(progressHandler,progress,error,stop,info);
    };
    PHAsset *phAsset = (PHAsset *)asset;
    ...
    [[PHImageManager defaultManager] requestAVAssetForVideo:phAsset options:options resultHandler:^(AVAsset *asset, AVAudioMix *audioMix, NSDictionary *info) {
        if(([asset isKindOfClass:[AVComposition class]] && ((AVComposition *)asset).tracks.count == 2)){
            ...
            NSString *tempDir = [root stringByAppendingString:@"/com.mxtrip.mxtrip/temp"];
            if (![[NSFileManager defaultManager] fileExistsAtPath:tempDir isDirectory:nil]) {
                [[NSFileManager defaultManager] createDirectoryAtPath:tempDir withIntermediateDirectories:YES attributes:nil error:nil];
            }
            NSString *myPathDocs =  [tempDir stringByAppendingPathComponent:[NSString stringWithFormat:@"Mxtrip-Video-%d.mov",arc4random() % 1000]];
...
- (DCMediaAssetModel *)assetModelWithAsset:(id)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    DCMediaAssetModel *model;
    DCMediaAssetModelMediaType type = DCMediaAssetModelMediaTypePhoto;
    
    PHAsset *phAsset = (PHAsset *)asset;
    if (phAsset.mediaType == PHAssetMediaTypeVideo)      type = DCMediaAssetModelMediaTypeVideo;
    else if (phAsset.mediaType == PHAssetMediaTypeAudio) type = DCMediaAssetModelMediaTypeAudio;
    else if (phAsset.mediaType == PHAssetMediaTypeImage) {
        if ([[phAsset valueForKey:@"filename"] hasSuffix:@"GIF"]) {
            type = DCMediaAssetModelMediaTypePhotoGif;
        }
    }
    
    if (!allowPickingImage && type == DCMediaAssetModelMediaTypePhotoGif) return nil;
    ...
    model = [DCMediaAssetModel modelWithAsset:asset type:type];
    return model;
}
...
- (NSPredicate *)configurePredicateWithAllowImage:(BOOL)image allowVideo:(BOOL)video range:(DCVideoDurationRange)range {
...
- (NSInteger)orientationFromImage:(UIImage *)image {
